...
# date            :2017-12-21
# version         :3.0
# usage           :In Maya obj_and_piv_to_origin.obj_and_piv_to_origin()
# notes           :
...
def obj_and_piv_to_origin():
    ...
    if meshes:
        ...
        if cmds.upAxis(q=True, axis=True) == 'y':
            ...
            # determines the distance to the origin from the current location in
            # worldspace of the pivot
            distance_to_origin = [-((bounding_box[0] + bounding_box[3]) / 2), \
                -(bounding_box[1]), -((bounding_box[2] + bounding_box[5]) / 2)]
